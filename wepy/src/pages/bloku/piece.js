...
var classNames = ['blue', 'orange', 'pink', 'green'];
var _ = {
...
export function makeAreaSquares(originArea) {
    if (originArea && originArea.length) {
        let area = [];
        for (let i = 0; i < 20; i++) {
            let arr = [];
            for (let j = 0; j < 20; j++) {
                arr.push({
                    className: originArea[i][j].className || '',
                    id: j + i * 20,
                    text: originArea[i][j].text || '',
                })
            }
            area.push(arr);
        }
...
export function getPieces(side, baseIds) {
    let pieces = [];
    basePieces.map((p, i) => {
        if (baseIds.includes(i)) {
...
function remove_touching(unoc, area, currentColor) {
    var nunoc = [];
    for (var i = 0; i < unoc.length; i++) {
        ...
        unoc[i].forEach(v => {
            ...
            touching.forEach(v2 => {
                ...
                if (b == currentColor) { //player_color 等于
                    flag = true;
                    return false;
                }
            });
...
function check_corners(unoc, area, currentColor) {
...
// 查看是否还有空余的位置可放
export function check_spaces(pieces, area, currentColor) {
    console.log(currentColor);
    var maxlen = 0;
    var unoc = check_corners(remove_touching(check_board(area), area, currentColor), area, currentColor);
    for (var i = 0; i < unoc.length; i++) {
...
export function check_valid_placement(piece, area, currentColor) {
    var flag = false;
